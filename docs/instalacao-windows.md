...
- `Lume-0.2.1-Windows-x64-Setup.exe` — opção recomendada;
- `Lume-0.2.1-Windows-x64.zip` — opção portátil;
...
O resultado esperado é `Lume 0.2.1`. PowerShell, CMD e o terminal integrado do
...
```powershell
Get-FileHash .\Lume-0.2.1-Windows-x64-Setup.exe -Algorithm SHA256
Get-FileHash .\Lume-0.2.1-Windows-x64.zip -Algorithm SHA256
```
